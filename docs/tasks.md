...
Goal: A facilitator can trigger AI extraction for a specific audio/video `MeetingUpload` whose transcription has completed (`POST /projects/{project_id}/cycles/{cycle_id}/uploads/{upload_id}/extract`), turning its `transcript_text` into draft `Decision` rows, draft `ActionItem` rows, and a cycle-level summary, all via a single mockable extraction function -- so this task never needs an AI/SDK dependency and no test ever makes a real network call. Every row this task creates is unconfirmed (`confirmed: false`) and the summary it writes is not yet shown anywhere team-facing; #21 owns turning any of it into the permanent record.
Depends on: #2 (Core data model), #16 (Record notes, decisions, and action items during discussion), #19 (Transcript generation from uploaded audio/video)
Acceptance Criteria:
- [ ] A facilitator can trigger extraction for an upload whose `status` is `complete` (`POST /projects/{project_id}/cycles/{cycle_id}/uploads/{upload_id}/extract`); when the injectable extraction function returns a result, the endpoint: creates one `Decision` row per returned decision string (`cycle_id` set, `cluster_id: null`, `confirmed: false`, `author_id: null`), creates one `ActionItem` row per returned action item (`cycle_id` set, `cluster_id: null`, `confirmed: false`, `status: "open"`, `due_date` from the result or `null`, `owner_id` resolved per the owner-matching rule below or `null`), sets `cycle.ai_summary` to the returned summary text, commits, and responds `200` with `{"status": "extracted", "summary": "...", "decisions": [...], "action_items": [...]}`, where each decision/action-item entry contains the same fields as its row (`id`, `cycle_id`, `cluster_id`, `description`, `confirmed`, `author_id`/`owner_id`, plus `due_date`/`status` for action items, `created_at`)
- [ ] An action item's `owner_email`, when present in the extraction result, is resolved to `owner_id` by an exact-match lookup (`User.email == owner_email`, mirroring #6's existing add-member-by-email lookup) AND a `ProjectMembership` for that user on this `project_id`; when both hold, `owner_id` is set to that user's id -- when either fails (no matching `User`, or a matching `User` with no membership on this project), `owner_id` is left `null` rather than rejecting the request or the individual item, since this is a best-effort match against free text, unlike #16's strict `404`-on-invalid-owner validation for a human-entered `owner_id`
- [ ] An action item's `due_date`, when present in the extraction result, is stored as-is on the row; when absent, `due_date` is `null`
- [ ] A returned decision string, or a returned action item's `description`, that is blank or whitespace-only is skipped (not persisted) rather than creating an empty row; a test proves a result mixing valid and blank entries persists only the valid ones
- [ ] `cycle.ai_summary` is overwritten with the newly extracted summary text on every successful call, including a call after a prior successful call already set it -- there is no accumulation or history of prior summaries
- [ ] When the injectable extraction function raises (simulating a failed or timed-out AI provider call), the endpoint catches the exception, creates no `Decision` or `ActionItem` rows, leaves `cycle.ai_summary` unchanged (`null` if never successfully set before, or whatever value a prior successful call left it as), and still responds `200` (never `4xx`/`5xx`) with `{"status": "unavailable", "summary": null, "decisions": [], "action_items": []}`
- [ ] A test stubs/monkeypatches the extraction function to return a fixed `ExtractionResult` and asserts the "extracted" behavior above, with no real network call and no API key/credential present
- [ ] A test stubs/monkeypatches the extraction function to raise and asserts the "unavailable" behavior above, again with no real network call, and proves no `Decision`/`ActionItem` rows exist afterward and `cycle.ai_summary` is unchanged
- [ ] Extraction can be triggered more than once on the same `complete` upload; a test calls it twice with different stubbed results and proves the second call's decisions/action items are created in addition to the first call's (not replacing or deduplicating them), while `cycle.ai_summary` reflects only the second call's summary
- [ ] Calling extract on an upload whose `status` is `pending`, `processing`, or `failed` is rejected with `409`, and no `Decision`/`ActionItem` rows are created and `cycle.ai_summary` is left unchanged
- [ ] Calling extract on an `upload_id` that does not exist, or that belongs to a different `cycle_id`/`project_id` than the ones in the path, is rejected with `404`
- [ ] Calling extract on a `cycle_id` that does not exist, or that belongs to a different `project_id` than the one in the path, is rejected with `404`, matching #18/#19's existing cycle-lookup pattern
- [ ] A request to extract from a user who holds only the `team_member` role on that project is rejected with `403`, via `require_role("facilitator")`
- [ ] A request to extract without a valid auth token is rejected with `401`
- [ ] A request to extract from a user who is authenticated but has no `ProjectMembership` on that project at all is rejected with `403`, not `500`
- [ ] A fresh `GET /projects/{project_id}/cycles/{cycle_id}/uploads` (#18) is unaffected by extraction -- this task adds no field to that response and does not change upload `status`
- [ ] `uv run pytest` passes, including the new tests covering the cases above
Out of scope:
- The facilitator review/edit/confirm/discard workflow for AI-drafted decisions, action items, and the summary -- that's #21's job, which depends on this task for the unconfirmed rows and `cycle.ai_summary` value it reviews
- Displaying `cycle.ai_summary`, or any confirmed decision/action item this task creates, on any team-facing page -- #22 (Retrospective summary page, not yet groomed) is expected to read `cycle.ai_summary` (post-#21 confirmation) for its own summary section; flagged here since #22 doesn't exist yet to confirm it, but no new column is needed for #22 to build on
- Extracting from `transcript_file` or `transcript_text` uploads -- neither kind can ever reach `status: "complete"` today (#19 only transcribes `audio`/`video`; #18's and #19's own grooming already flagged that nothing in the backlog advances a `transcript_file`/`transcript_text` upload's status off `pending`), so this task's `status == "complete"` gate naturally excludes them without a separate `kind` check. No new follow-up filed since #19 already flagged the underlying gap
- Which AI/LLM provider to call, prompt design, and API key/credential management -- treated as a config/environment concern, mirroring #12's and #19's identical Out-of-scope item; the injectable function in Constraints is the extension point a future provider gets wired in behind
- A background job / async task queue for the extraction call -- despite the original issue's "background job" description, this project has no job infrastructure (no Celery/queue), mirroring #12's and #19's identical reasoning; the call stays synchronous within this endpoint's own request
- Matching an action item owner by anything other than an exact `User.email` match (e.g. fuzzy name matching against a spoken name in the transcript) -- the extraction function's output already commits to an `owner_email` field (see Constraints); resolving a spoken name to an email is the extraction function's problem, not this endpoint's
- A frontend UI for triggering extraction or reviewing results -- consistent with #4-#19, no template/static layer exists in the project and nothing later in the plan calls for one, so no follow-up filed
- Deleting or discarding a previously created draft directly through this task's endpoint -- reviewing/discarding drafts is #21's job; this task only ever adds rows
Constraints:
- Depends on #2 (Core data model), #16 (Record notes, decisions, and action items during discussion), and #19 (Transcript generation from uploaded audio/video) being merged. #16 is groomed but not yet implemented as of this grooming pass: it adds `Decision.author_id` and `ActionItem.owner_id` as `nullable=False` columns for its own always-populated manual-entry path; this task depends on those columns already existing and, via its own migration (see below), relaxes both to `nullable=True` to accommodate AI-drafted rows that have no human author/owner. If #16 is still unmerged when this task starts, tests needing those columns may seed a cycle/upload directly via the ORM, consistent with how #19's tests seed a `pending` upload ahead of #18 landing
- Add a new Alembic migration (`uv run alembic revision --autogenerate -m "..."`, matching whichever file is newest in `migrations/versions/` at implementation time) that: (a) adds `ai_summary: Mapped[str | None] = mapped_column(Text)` to `FeedbackCycle` in `app/models.py` -- one summary per cycle, overwritten on each successful extraction, no new table; (b) alters `Decision.author_id` and `ActionItem.owner_id` to `nullable=True`, since #16 declared both `nullable=False` for its own manual-entry-only case and this task is the first to need a row with neither set
- The extraction call must go through a single, mockable function in a new `app/ai_extraction.py` module, mirroring #12's `app/ai_clustering.py`/`generate_cluster_suggestions` and #19's `app/transcription.py`/`transcribe_audio` pattern exactly: `extract_decisions_and_actions(transcript_text: str) -> ExtractionResult` raises `NotImplementedError` unconditionally since no provider is wired up, the endpoint catches all exceptions it raises, and tests must monkeypatch/stub it so no test makes a real network call or needs a real API key/credential in CI
- `ExtractionResult` is a dataclass with `decisions: list[str]`, `action_items: list[ExtractedActionItem]`, and `summary: str`; `ExtractedActionItem` is a dataclass with `description: str`, `owner_email: str | None = None`, and `due_date: date | None = None` -- typed fields (a real `date`, not a string to parse) avoid format-parsing edge cases entirely, matching #12's `SuggestedGroup` dataclass style
- No AI/LLM SDK dependency is added to `pyproject.toml` in this task -- the interface is provider-agnostic; wiring a real provider behind it is a future concern per Out of scope
- The endpoint's checks run in this order: cycle exists and belongs to the given `project_id` (`404` otherwise); upload exists and belongs to the given `cycle_id`/`project_id` (`404` otherwise); `upload.status == UploadStatus.COMPLETE` (`409` otherwise). No separate check that `transcript_text` is non-null is needed: #19 guarantees `transcript_text` is populated whenever `status == COMPLETE`, and nothing in the backlog can violate that invariant
- Use `Depends(require_role("facilitator"))` for the route, matching #18's and #19's precedent that mutating a cycle-scoped upload/transcript-derived resource is facilitator-only
- Add the new route to the existing `app/uploads.py` router (from #18/#19) rather than a new router module, following its existing Pydantic response-model / `Depends(get_db)` / `HTTPException(status_code=..., detail=...)` pattern
- Re-extraction is allowed and deliberately additive: calling extract again on the same `complete` upload creates a new batch of draft rows rather than replacing or deduplicating the prior batch, and overwrites `cycle.ai_summary` with the new summary -- simpler than tracking "already extracted", and #21's facilitator review already handles discarding drafts it doesn't want
- All rows and the summary write happen in a single transaction: on success, all `Decision`/`ActionItem` inserts and the `cycle.ai_summary` update commit together; on any exception from the extraction function, nothing is written -- no partial batch, no partial summary update
- Follow the existing FastAPI pattern in `app/cycles.py`/`app/uploads.py` for cycle/upload lookup and `404`, and `HTTPException(status_code=..., detail=...)` for errors
- Add tests under `tests/`, following the `TestClient` + in-memory-SQLite pattern already used in `tests/test_auth.py`, and #12/#19's monkeypatch style for stubbing the injectable function
